...
/summon minecraft:zombie ~ ~ ~ {
  data:{dvh:{max_health:40d, health:40d}},
  NoAI:1b
}
...
| 函数                             | 说明                                          | 调用方式 |
| ------------------------------ | ------------------------------------------- | -------- |
| `api/create`                   | 注入虚拟血量                                    | `{with:{max_health, health, on_death?}}` |
| `api/remove`                   | 剥离虚拟血量（可选击杀）                            | `{with:{kill:1b/0b}}` |
| `api/add_health`               | 增减血量，正=治疗/负=伤害（无敌拦截负值）                | `{points:int}` |
| `api/set_health`               | 强制设定血量（HP），≤0 触发死亡                      | `{health:int}` |
| `api/set_max_health`           | 设定最大血量（HP）                                 | `{max_health:int}` |
| `api/add_max_health`           | 增减最大血量（HP）                                 | `{points:int}` |
| `api/get_health`               | 导出当前血量（HP）                                | `execute store result score #o obj run ...` |
| `api/get_health_mhp`           | 导出当前血量（mHP）                               | `execute store result score #o obj run ...` |
| `api/get_max_health`           | 导出最大血量（HP）                                | `execute store result score #o obj run ...` |
| `api/get_max_health_mhp`       | 导出最大血量（mHP）                               | `execute store result score #o obj run ...` |
| `api/get_percentage`           | 导出当前血量百分比 [0,100]                         | `execute store result score #o obj run ...` |
| `api/get_stats`                | 导出统计数据 ÷scale                               | `{damage_out, healing_out, scale}` |
| `api/set_invulnerable`         | 设置无敌状态                                      | `{state:1b/0b}` |
| `api/set_damage_mult`          | 设置伤害比例 (×1000, 默认1000=100%)                | `{mult:int}` |
...
自动补全（需 Spyglass + mcdoc）：`data:{dvh:{max_health:0d, health:0d}}`
